...
for t in range(numFrames):   # for each time frame
    ...
    while k<iterations and abs(currCircSum)>10**(-14):
        ...
        # calculate velocity at wake panel, include heaving motion and pitching motion
        if t>0:
            uWake=np.sum(xVPW)*x[-1]+np.dot(xSPW,x[:-1])+np.dot(xVVW,vortStrength[:t])+Uinf*np.cos(theta_t[t])+h_t_dot[t]*np.sin(theta_t[t])
            vWake=np.sum(yVPW)*x[-1]+np.dot(ySPW,x[:-1])+np.dot(yVVW,vortStrength[:t])+Uinf*np.sin(theta_t[t])-h_t_dot[t]*np.cos(theta_t[t])+theta_t_dot[t]*chord/2
        else:
            uWake=np.sum(xVPW)*x[-1]+np.dot(xSPW,x[:-1])+Uinf*np.cos(theta_t[t])+h_t_dot[t]*np.sin(theta_t[t])
            vWake=np.sum(yVPW)*x[-1]+np.dot(ySPW,x[:-1])+Uinf*np.sin(theta_t[t])-h_t_dot[t]*np.cos(theta_t[t])+theta_t_dot[t]*chord/2
    ...
    if t>numFrames*1//3-1 and t%5==0:
        ...
        title='k = ' + '{:.3f}'.format(f*chord/Uinf) + 't/T = ' + '{:.3f}'.format((tInterval[t]-1)/.5) + r'   $\theta_p$ = ' + '{:.3f}'.format(theta_t[t]*180/np.pi) + r'$^\circ$' + r'   $C_Y = $' + '{:.3f}'.format(float(CySto[n]))
        ...
        fig.savefig(pathName + '//_frame' + str(n) + '.png')
        plt.close()
        n=n+1
