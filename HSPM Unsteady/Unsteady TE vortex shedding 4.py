...
for t in range(numFrames):   # for each time frame
    ...
    wakePanelLen=tStep*(Uinf*np.cos(theta_t[t])+foilContNorm[-1]*-np.sin(theta[-1])+foilContTang[-1]*np.cos(theta[-1]))*.25#!!!!!!!!!!!!!!!!!!!!!
    ...
    while k<iterations and abs(currCircSum)>10**(-10):
        ...
        xVPW=tTemp*np.cos(wakePanelTheta)-nTemp*np.sin(wakePanelTheta)
        yVPW=tTemp*np.sin(wakePanelTheta)+nTemp*np.cos(wakePanelTheta)
        ...
        if t>0:
            uWake=np.sum(xVPW)*x[-1]+np.dot(xSPW,x[:-1])+np.dot(xVVW,vortStrength[:t])+Uinf*np.cos(theta_t[t])+(h_t[t+1]-h_t[t])*np.sin(theta_t[t])
            vWake=np.sum(yVPW)*x[-1]+np.dot(ySPW,x[:-1])+np.dot(yVVW,vortStrength[:t])+Uinf*np.sin(theta_t[t])-(h_t[t+1]-h_t[t])*np.cos(theta_t[t])
        else:
            uWake=np.sum(xVPW)*x[-1]+np.dot(xSPW,x[:-1])+Uinf*np.cos(theta_t[t])+(h_t[t+1]-h_t[t])*np.sin(theta_t[t])
            vWake=np.sum(yVPW)*x[-1]+np.dot(ySPW,x[:-1])+Uinf*np.sin(theta_t[t])-(h_t[t+1]-h_t[t])*np.cos(theta_t[t])
    ...
    plt.xlim(-.5,1.5)
    plt.ylim(-1,1)
    plt.plot(xpGlobal,ypGlobal)
    plt.plot(xcWakeGlobal,ycWakeGlobal,'o')
    if t>0:
        plt.plot(xVorPosGlobal[:t],yVorPosGlobal[:t],'*')
