    ...
    if t>0:
        ...
        for i in range(numPanels):
            for j in range(t):
                ...
                nVVP[i,j]=-np.cos(theta[i]-theta_im)/(2*np.pi*r1)
        ...
        for i in range(t):
            for j in range(t):
                if i==j:
                    ...
                    yVV[i,j]=-np.cos(-theta_im)/(2*np.pi*r1)
    ...
    while k<iterations:
        ...
        if t>0:
            ...
            for i in range(t):
                ...
                nVVW[0,i]=-np.cos(wakePanelTheta-theta_im)/(2*np.pi*r1)
        ...
        tWP=np.cos(theta-wakePanelTheta)*xTemp+np.sin(theta-wakePanelTheta)*yTemp
        ...
        if t==0:
            ...
            currGamma=prevGamma+tStep*(tangVelSto[0,t-1]**2-tangVelSto[-1,t-1]**2)/(2*perimeter) #MAYBE CHANGE THIS...
